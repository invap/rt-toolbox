...
def build_rabbitmq_server_connections(file_path):
    global rabbitmq_result_log_server_connection
    try:
        f = open(file_path, "rb")
    ...
    try:
        ...
        exchange = result_log_conf_dict["name"] if "name" in result_log_conf_dict else "events_exchange"
        ...
        rabbitmq_result_log_server_connection = RabbitMQ_server_incoming_connection(
            server_info,
            connection_attempts,
            retry_delay,
            exchange,
